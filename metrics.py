# reference : https://github.com/microsoft/NeuralSpeech/blob/master/BinauralGrad/src/binauralgrad/losses.py
# modify based on current torch version
import numpy as np
import torch as th
...
class AmplitudeLoss(Loss):
    ...
    def _loss(self, data, target):
        ...
        return th.mean(th.abs(pred_mag - target_mag))
...
class PhaseLoss(Loss):
    ...
    def _to_complex(self, spec):
        if th.is_complex(spec):
            return spec

        # old format: [..., 2] = [real, imag]
        if spec.shape[-1] == 2:
            return th.view_as_complex(spec.contiguous())
    ...
    def _loss(self, data, target):
        ...
        :return: a scalar loss value
        '''
        data_spec = self._to_complex(self._transform(data))
        target_spec = self._to_complex(self._transform(target))

        # flatten all B, C, F, time bins
        data_spec = data_spec.reshape(-1)
        target_spec = target_spec.reshape(-1)

        # ignore low-energy components for numerical stability
        target_energy = target_spec.abs()
        pred_energy = data_spec.detach().abs()

        threshold = self.ignore_below * target_energy.mean()

        target_mask = target_energy > threshold
        pred_mask = pred_energy > threshold
        mask = target_mask & pred_mask

        if mask.sum() == 0:
            return data_spec.real.new_tensor(0.0)

        data_spec = data_spec[mask]
        target_spec = target_spec[mask]

        # shortest angular distance between predicted phase and target phase
        # angle(pred * conj(target)) gives phase(pred) - phase(target), wrapped to [-pi, pi]
        phase_diff = th.angle(data_spec * target_spec.conj())

        loss = phase_diff.abs().mean()
        return loss